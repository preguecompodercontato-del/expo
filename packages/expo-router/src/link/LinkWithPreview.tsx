...
interface LinkMenuActionProps {
   ...
   *
   * This is marked as dangerous, because when action is selected it will recreate the menu,
   ...
   */
  dangerouslyKeepPresented?: boolean;
  /**
...
export function LinkMenuAction(props: LinkMenuActionProps) {
  ...
  const { dangerouslyKeepPresented, onPress, ...rest } = props;
  return (
    <NativeLinkPreviewAction
      {...rest}
      onSelected={onPress}
      keepPresented={dangerouslyKeepPresented}
    />
  );
}
...
export const LinkMenu: FC<LinkMenuProps> = (props) => {
  ...
  return <NativeLinkPreviewAction {...props} title={props.title ?? ''} onSelected={() => {}} />;
};
